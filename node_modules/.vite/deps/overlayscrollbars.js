...
var createOverflowUpdateSegment = (t2, s2) => {
  ...
  const { vt: c2, rn: r2, L: i2, V: a2, Kt: u2, gt: f2, bt: _2, yt: d2, an: p2 } = t2;
  const { U: v2 } = e2;
  const g2 = _2 && a2;
  const h2 = bind(o, 0);
  const b2 = {
  ...
  const [m2, O2] = createCache(y2, bind(getFractionalSize, i2));
  const [C2, $2] = createCache(y2, bind(D, i2));
  const [z2, I2] = createCache(y2);
  const [A2] = createCache(S2);
  const [M2, k2] = createCache(y2);
  ...
  return ({ zt: n2, Qt: o2, un: l2, At: a3 }, { _n: u3 }) => {
    const { _t: f3, Ht: _3, xt: b3, Ct: w3, ft: y3, Et: S3 } = o2 || {};
    const x2 = P2 && P2.R(t2, s2, l2, e2, n2);
    const { X: H2, Y: E2, W: D2 } = x2 || {};
    const [q2, F2] = getShowNativeOverlaidScrollbars(n2, e2);
    const [j2, X2] = n2("overflow");
    const Y2 = overflowIsVisible(j2.x);
    const W2 = overflowIsVisible(j2.y);
    const J2 = f3 || u3 || b3 || w3 || S3 || F2;
    let G2 = O2(a3);
    let Q2 = $2(a3);
    let tt2 = I2(a3);
    let nt2 = k2(a3);
    ...
    if (J2) {
      ...
      nt2 = M2(l3);
      tt2 = z2(getOverflowAmount(r3, l3), a3);
    }
    const [ot2, st2] = nt2;
    const [et2, ct2] = tt2;
    const [rt2, it2] = Q2;
    ...
    const dt2 = u3 || w3 || S3 || at2 || it2 || st2 || ct2 || X2 || F2 || J2 || _3 && g2;
    const [pt2, vt2] = V2(a3);
    const gt2 = w3 || y3 || vt2 || ft2 || a3;
    const [ht2, bt2] = gt2 ? L2(getMeasuredScrollCoordinates(pt2), a3) : U2();
    let wt2 = getViewportOverflowStyle(ut2, j2);
    setMeasuringMode(false);
    if (dt2) {
      setViewportOverflowStyle(wt2);
      wt2 = getElementOverflowStyle(i2, ut2);
      if (D2 && H2) {
        H2(wt2, rt2, lt2);
        setStyles(i2, D2(wt2));
      }
    }
    const [yt2, St2] = R2(wt2);
    addRemoveAttrClass(c2, B, N, _t2);
    addRemoveAttrClass(r2, Z, N, _t2);
    assignDeep(s2, {
      cn: yt2,
      Vt: {
        x: ot2.w,
      ...
      j: ut2,
      Dt: sanitizeScrollCoordinates(ht2, et2)
    });
    return {
      sn: St2,
      tn: st2,
